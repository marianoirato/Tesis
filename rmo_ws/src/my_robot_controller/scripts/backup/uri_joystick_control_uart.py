...
class SerialTransmitter:
    ...
    def send_velocity(self, vx, vy, wz):
        ...
        framed_data = struct.pack('B',START_BYTE) + data + struct.pack('B',END_BYTE)    # Agrega el byte de cabecera y el byte de fin de trama
       ...
       # Convertir framed_data a una lista de enteros para la representación binaria
        framed_data_integers = [byte for byte in framed_data]
        print("framed_data (bytes):", framed_data_integers)

        # Convertir cada byte a su representación binaria
        framed_data_binary = ' '.join(format(byte, '08b') for byte in framed_data_integers)
        print("framed_data (binario):", framed_data_binary)
...
def joy_callback(data):
    ...
    if vx >= (vx_anterior + 0.1) or vy >= (vy_anterior + 0.1) or wz >= (wz_anterior + 0.1):
